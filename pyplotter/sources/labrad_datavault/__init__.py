...
from typing import Dict, Tuple, List, Union, Optional, final
...
def getNbTotalRunmp(databaseAbsPath: str) -> None:
...
class Session(object):
    ...
    def load(self):
        ...
        if self.counter != len(self.listDatasetNums()) + 1:
            print("update counter based on existing number of datasets!")
            self.counter = len(self.listDatasetNums()) + 1
    ...
    def listContents(self, tagFilters):
        ...
        csv_datasets = [
            filename_decode(s[:-4])
            for s in files
            if s.endswith(".ini") and s.lower() != "session.ini"
        ]
    ...
    def openDataset(self, name, write_access=False, noisy=True):
        name = self.fileName(name)
        if write_access:
            if name in self.datasets:
                print("close existing file", self.datasets[name])
                self.datasets[name].data.file.close()
            # need to create a new wrapper for this dataset
            dataset = Dataset(self, name, write_access=write_access, noisy=noisy)
            if not dataset.data.file.swmr_mode:
                dataset.data.file.swmr_mode = True
            self.datasets[name] = dataset
        elif name in self.datasets:
            dataset = self.datasets[name]
            # dataset.access()  # can not update access attribute
        else:
            dataset = Dataset(self, name, noisy=noisy)
            self.datasets[name] = dataset
        dataset.data.dataset.refresh()
        self.access()
        return dataset

    def closeDataset(self, name):
        name = self.fileName(name)
        dataset = self.datasets[name]
...
class Dataset(object):
    ...
    def __init__(
        ...
        if create:
            ...
            print(f"access {name} {write_access}") if noisy else None
            self.data = backend.open_backend(file_base, write_access=write_access)
            self.write_access = write_access
            self.load()
            self.access() if write_access else None
    ...
    def access(self):
        ...
        # if read only, do not changing access -XL
        self.data.access() if self.write_access else None
        self.save()
